...
from config import cfg, assert_and_infer_cfg # import引入的是变量和函数
from utils.misc import AverageMeter, prep_experiment, evaluate_eval, fast_hist, campusE1_fast_hist
import datasets
import loss
import network
import optimizer
...
# Enable CUDNN Benchmarking optimization 启动Torch-GPU、CUDA优化（主要是随机数种子）
#torch.backends.cudnn.benchmark = True
random_seed = cfg.RANDOM_SEED  #304
torch.manual_seed(random_seed)
torch.cuda.manual_seed(random_seed)
torch.cuda.manual_seed_all(random_seed) # if use multi-GPU，为每个gpu生成随机数种子
torch.backends.cudnn.deterministic = True # 固定随机数种子是非常重要的 (TORCH + CUDA)
...
def main():
    ...
    _, _, _, extra_val_loaders, _ = datasets.setup_loaders(args) # 选取数据集

    criterion, criterion_val = loss.get_loss(args) # 创建Loss（Standard cross entropy or WT loss）
    criterion_aux = loss.get_loss_aux(args)
    net = network.get_net(args, criterion, criterion_aux) # 创建网络

    optim, scheduler = optimizer.get_optimizer(args, net) # 创建优化器设置
    ...
    for val_idx, data in enumerate(val_loader): # 一张一张来的
        ...
        with torch.no_grad():
            if args.use_wtloss:
                ...
                output = net(inputs, dsbn=args.dsbn, mode='val', domain_label=domain_label) # 推理
        ...
        if "campuse1" == args.val_dataset[0]:
            num_classes = 10
        else:
