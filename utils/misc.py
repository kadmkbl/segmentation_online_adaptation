...
def fast_hist(label_pred, label_true, num_classes): # target × nclass + pred
    ...
    __q = num_classes * _q
    b = label_pred[mask]
    a = __q + b # 对于10分类刚好：预测值是个位数，真实值是十位数
    """
    a = num_classes * label_true[mask].astype(int) + label_pred[mask]
    hist = np.bincount(a, minlength=num_classes ** 2)
...
def evaluate_eval_dur_train(args, net, optimizer, scheduler, val_loss, hist, dump_images, writer, epoch=0, dataset_name=None, dataset=None, curr_iter=0, optimizer_at=None, scheduler_at=None, save_pth=True):
    """
    Modified IOU mechanism for on-the-fly IOU calculations ( prevents memory overflow for
    large dataset) Only applies to eval/eval.py
    """
    if val_loss is not None and hist is not None:
        ...
        _a = a.sum()
        b = hist.sum()
        c = hist.sum(axis=1) # 行之和
        d = hist.sum(axis=0) # 列之和
        ...
        acc_cls = np.diag(hist) / hist.sum(axis=1)
        acc_cls = np.nanmean(acc_cls)
        iu = np.diag(hist) / (hist.sum(axis=1) + hist.sum(axis=0) - np.diag(hist)) # 包含了每一类的IOU（理解语义分割中的IOU怎么计算的）
        
        print_evaluate_results(hist, iu, dataset_name=dataset_name, dataset=dataset)
        freq = hist.sum(axis=1) / hist.sum()
        mean_iu = np.nanmean(iu) # MIOU是每个类别IOU的平均值（也就是说与各个类别的像素点个数比例无关）
        logging.info('mean {}'.format(mean_iu))
        fwavacc = (freq[freq > 0] * iu[freq > 0]).sum()
    else:
        mean_iu = 0
    ...
    if val_loss is not None and hist is not None:
        ...
        if writer:
            ...
            writer.add_scalar('{}/mean_iu'.format(dataset_name), mean_iu, epoch)
            writer.add_scalar('{}/val_loss'.format(dataset_name), val_loss.avg, epoch)
    
    return iu, mean_iu

def save_best_acc(args, net, optimizer, scheduler, epoch, mean_iu, train_dataset_name):
    best_acc_name = 'best_{}_epoch_{}_mean-iu_{:.5f}_{}.pth'.format(
                        args.weather, epoch, mean_iu, train_dataset_name)
    best_acc_name = os.path.join(args.exp_path, best_acc_name)
...
def evaluate_eval_all(args, net, optimizer, scheduler, hist, writer, epoch=0, dataset_name=None, dataset=None, curr_iter=0, optimizer_at=None, scheduler_at=None, save_pth=True):
    """
    Modified IOU mechanism for on-the-fly IOU calculations ( prevents memory overflow for
    large dataset) Only applies to eval/eval.py
    """
    ...
    if save_pth:
        ...
        if hist is not None:
            if dataset_name not in args.best_record.keys():
                args.best_record[dataset_name] = {'epoch': -1, 'iter': 0, 'acc': 0,
                                                  'acc_cls': 0, 'mean_iu': 0, 'fwavacc': 0}
            # update best snapshot
            if mean_iu > args.best_record[dataset_name]['mean_iu'] :
                # remove old best snapshot
    ...
    if hist is not None:
        ...
        if save_pth:
            fmt_str = 'best record: [dataset name %s], [val loss %.5f], [acc %.5f], [acc_cls %.5f], ' +\
                      '[mean_iu %.5f], [fwavacc %.5f], [epoch %d], '
            logging.info(fmt_str % (dataset_name,
                                    args.best_record[dataset_name]['acc'],
                                    args.best_record[dataset_name]['acc_cls'], args.best_record[dataset_name]['mean_iu'],
                                    args.best_record[dataset_name]['fwavacc'], args.best_record[dataset_name]['epoch']))
            logging.info('-' * 107)
