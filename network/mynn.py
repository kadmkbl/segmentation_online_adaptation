...
def Norm2d(in_channels):
    ...
    """
    layer = getattr(cfg.MODEL, 'BNFUNC') # 这里只是选择时syncBN或BN
...
def forgiving_state_restore(net, loaded_dict):
    ...
    new_loaded_dict = {}
    if 'arch' in loaded_dict:
        if loaded_dict['arch'] == 'erfnet':
            loaded_dict = loaded_dict['state_dict']
            for k in net_state_dict:
                k_ = 'module.features.' + k
                if k_ in loaded_dict and net_state_dict[k].size() == loaded_dict[k_].size():
                    new_loaded_dict[k] = loaded_dict[k_]
                # else:
