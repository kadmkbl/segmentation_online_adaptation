...
class _BNReluConv_adapter(nn.Sequential):
    tasks_preset = 100

    def __init__(self, num_maps_in, num_maps_out, k=3, batch_norm=True, bn_momentum=0.1, bias=False, dilation=1):
        super(_BNReluConv_adapter, self).__init__()
        if batch_norm:
            self.bns = nn.ModuleList([nn.BatchNorm2d(num_maps_in, momentum=bn_momentum) for i in range(self.tasks_preset)])
        self.relu = nn.ReLU(inplace=batch_norm is True)
        padding = k // 2
        self.conv = nn.Conv2d(num_maps_in, num_maps_out, kernel_size=k, padding=padding, bias=bias, dilation=dilation)
        self.parallel_conv = nn.ModuleList([conv1x1(num_maps_in, num_maps_out) for i in range(self.tasks_preset)])

    def forward(self, x, task=None):
        out = self.bns[task](x)
        out = self.relu(out)
        out = self.conv(out) + self.parallel_conv[task](out)
...
class _Upsample_adapter(nn.Module):
    def __init__(self, num_maps_in, skip_maps_in, num_maps_out, use_bn=True, k=3):
        super(_Upsample_adapter, self).__init__()
        print(f'Upsample layer: in = {num_maps_in}, skip = {skip_maps_in}, out = {num_maps_out}')
        self.bottleneck = _BNReluConv_adapter(skip_maps_in, num_maps_in, k=1, batch_norm=use_bn)
        self.blend_conv = _BNReluConv_adapter(num_maps_in, num_maps_out, k=k, batch_norm=use_bn)

    def forward(self, x, skip, task=None):
        skip = self.bottleneck.forward(skip, task=task)
        skip_size = skip.size()[2:4]
        x = upsample(x, skip_size)
        x = x + skip
        x = self.blend_conv.forward(x, task=task)
        return x
...
class SpatialPyramidPooling_adapter(nn.Module):
    def __init__(self, num_maps_in, num_levels, bt_size=512, level_size=128, out_size=128,
                 grids=(6, 3, 2, 1), square_grid=False, bn_momentum=0.1, use_bn=True):
        super(SpatialPyramidPooling_adapter, self).__init__()
        self.grids = grids
        self.square_grid = square_grid
        self.spp = nn.Sequential()
        self.spp.add_module('spp_bn',
                            _BNReluConv_adapter(num_maps_in, bt_size, k=1, bn_momentum=bn_momentum, batch_norm=use_bn))
        num_features = bt_size
        final_size = num_features
        for i in range(num_levels):
            final_size += level_size
            self.spp.add_module('spp' + str(i),
                                _BNReluConv_adapter(num_features, level_size, k=1, bn_momentum=bn_momentum, batch_norm=use_bn))
        self.spp.add_module('spp_fuse',
                            _BNReluConv_adapter(final_size, out_size, k=1, bn_momentum=bn_momentum, batch_norm=use_bn))
    ...
    def forward(self, x, task=None):
        ...
        x = self.spp[0].forward(x, task=task)
        levels.append(x)
        num = len(self.spp) - 1
        ...
        for i in range(1, num):
            if not self.square_grid:
               ...
               x_pooled = F.adaptive_avg_pool2d(x, self.grids[i - 1])
            level = self.spp[i].forward(x_pooled, task=task)

            level = upsample(level, target_size)
            levels.append(level)
        x = torch.cat(levels, 1)
        x = self.spp[-1].forward(x, task=task)
        return x
