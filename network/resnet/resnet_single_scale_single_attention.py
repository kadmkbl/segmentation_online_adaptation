...
from ..util import _Upsample, _Upsample_adapter, SpatialPyramidPooling, SpatialPyramidPooling_adapter
...
def _bn_function_factory_adapter(conv, parallel_convs, norms, relu=None, task=None):
    """return a conv-bn-relu function"""
    def bn_function(x):
        out = conv(x)
        if parallel_convs is not None:
            out = out + parallel_convs[task](x)
...
class BasicBlock_adapter(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, efficient=False, use_bn=True):
        super(BasicBlock_adapter, self).__init__()
        self.use_bn = use_bn
        self.conv1 = conv3x3(inplanes, planes, stride)
        self.parallel_conv1 = nn.ModuleList([conv1x1(inplanes, planes, stride) for i in range(self.tasks_preset)])
        self.bns1 = nn.ModuleList([nn.BatchNorm2d(planes) for i in range(self.tasks_preset)]) if self.use_bn else None
        # self.bn1 = nn.BatchNorm2d(planes) if self.use_bn else None
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
        self.parallel_conv2 = nn.ModuleList([conv1x1(planes, planes) for i in range(self.tasks_preset)])
        self.bns2 = nn.ModuleList([nn.BatchNorm2d(planes) for i in range(self.tasks_preset)]) if self.use_bn else None
        # self.bn2 = nn.BatchNorm2d(planes) if self.use_bn else None
        self.downsample = downsample
        self.stride = stride
        self.efficient = efficient
    ...
    def forward(self, x, task):
        ...
        bn_1 = _bn_function_factory_adapter(self.conv1, self.parallel_conv1, self.bns1, relu=self.relu, task=task)
        bn_2 = _bn_function_factory_adapter(self.conv2, self.parallel_conv2, self.bns2, task=task)
...
class ResNet_adapter(nn.Module):
    def __init__(self, block, layers, *, num_features=128, k_up=3, efficient=True, use_bn=True,
        ...
        self.attention_1 = self.attention(64)
        self.attention_1_d = self.attention(64)
        upsamples += [_Upsample_adapter(num_features, self.inplanes, num_features, use_bn=self.use_bn, k=k_up)] #  num_maps_in, skip_maps_in, num_maps_out, k: kernel size of blend conv

        # layer2~4 (specific adapters)
        self.layer2 = self._make_layer_rgb_adapter(BasicBlock_adapter, 64, 128, layers[1], stride=2)
        self.layer2_d = self._make_layer_d(block, 64, 128, layers[1], stride=2)
        self.attention_2 = self.attention(128)
        self.attention_2_d = self.attention(128)
        upsamples += [_Upsample_adapter(num_features, self.inplanes, num_features, use_bn=self.use_bn, k=k_up)]

        self.layer3 = self._make_layer_rgb_adapter(BasicBlock_adapter, 128, 256, layers[2], stride=2)
        self.layer3_d = self._make_layer_d(block, 128, 256, layers[2], stride=2)
        self.attention_3 = self.attention(256)
        self.attention_3_d = self.attention(256)
        upsamples += [_Upsample_adapter(num_features, self.inplanes, num_features, use_bn=self.use_bn, k=k_up)]

        self.layer4 = self._make_layer_rgb_adapter(BasicBlock_adapter, 256, 512, layers[3], stride=2)
        self.layer4_d = self._make_layer_d(block, 256, 512, layers[3], stride=2)
        self.attention_4 = self.attention(512)
        self.attention_4_d = self.attention(512)
        ...
        self.spp = SpatialPyramidPooling_adapter(self.inplanes, num_levels, bt_size=bt_size, level_size=level_size,
                                         out_size=self.spp_size, grids=spp_grids, square_grid=spp_square_grid,
                                         bn_momentum=0.01 / 2, use_bn=self.use_bn)
        self.upsample = nn.ModuleList(list(reversed(upsamples)))
    ...
    def _make_layer_rgb_adapter(self, block, inplanes, planes, blocks, stride=1):
        downsample = None
        if stride != 1 or inplanes != planes * block.expansion:
            layers = [nn.Conv2d(inplanes, planes * block.expansion, kernel_size=1, stride=stride, bias=False)]
            if self.use_bn:
    ...
    def forward_resblock_adapter(self, x, layers, task=None):
        skip = None
        for l in layers:
            x = l(x, task)
            if isinstance(x, tuple):
    ...
    def forward_down(self, rgb, task):
        x = self.conv1(rgb)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        pool2 = x
        features += [skip]
        x, skip = self.forward_resblock_adapter(x, self.layer2, task)
        features += [skip]
        x, skip = self.forward_resblock_adapter(x, self.layer3, task)
        features += [skip]
        x, skip = self.forward_resblock_adapter(x.detach(), self.layer4, task)
        features += [self.spp.forward(skip, task=task)]
        
        style_feats = [pool1, pool2]
        return features, style_feats, task
    ...
    def forward_up(self, features):
        ...
        for skip, up in zip(features[1:], self.upsample):
            ...
            #print(len(self.upsample))
            if i < len(self.upsample):
                x = up(x, skip, task=task)
            else:
                x = up(x, skip, task=task)
            upsamples += [x]
        return x, {'features': features, 'upsamples': upsamples}, style_feats

    def forward(self, rgb, depth=None, task=None):
        if depth is None:
            return self.forward_up(self.forward_down(rgb, task))
        else:
            return self.forward_up(self.forward_down_fusion(rgb, depth))
...
def resnet18_adapter(pretrained=True, **kwargs):
    """Constructs a ResNet-18_adapter model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_adapter(BasicBlock, [2, 2, 2, 2], **kwargs)
    """
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']), strict=False)
        print('pretrained dict loaded sucessfully')
