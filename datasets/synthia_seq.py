...
import datasets.synthia_seq_labels as synthia_seq_labels
import copy
import scipy.misc as m

from config import cfg

trainid_to_name = synthia_seq_labels.trainId2name
id_to_trainid = synthia_seq_labels.label2trainid
color_to_trainid = synthia_seq_labels.color2trainId
num_classes = 13
ignore_label = 255
root = cfg.DATASET.Synthia_SEQ_DIR
img_postfix = '.png'
...
def make_dataset(mode, weather, maxSkip=0, cv_split=0):
    ...
    assert weather in ['DAWN', 'FALL', 'all']
    img_dir_name = 'rgb'
    mask_postfix = '.png' # 标注的后缀
    if weather == "all":
        for weather in ['DAWN', 'FALL']:
        ...
        img_path = os.path.join(root, img_dir_name, 'rgb_anon', weather) # 图片所处地址（该目录下有train val test；接下来是各个videos；然后才是图片）
        mask_path = os.path.join(root, 'gt_trainval', 'gt', weather) # 标注所处地址（该目录下有train val；接下来是各个videos；然后才是标注）
        cv_splits = make_cv_splits(img_dir_name, weather) # 根据图片所处路径划分train和val的videos
        if mode == 'trainval':
            modes = ['train', 'val']
        else:
    ...
    # logging.info('Synthia_SEQ-{}: {} images'.format(mode, len(items)))
    logging.info('Synthia_SEQ-{}-{}: {} images'.format(mode, weather, len(items) + len(aug_items)))
    return items, aug_items


class STNTHIA_SEQ_Uniform(data.Dataset):
    """
    ...
    def __init__(self, mode, weather, maxSkip=0, joint_transform_list=None, sliding_crop=None,
        ...
        json_fn = 'Synthia_SEQ_{}_cv{}_tile{}_{}.json'.format(
...
class STNTHIA_SEQ(data.Dataset):
