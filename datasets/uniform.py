...
def class_centroids_image(item, tile_size, num_classes, id2trainid):
    ...
    for x_offs, y_offs in tile_locations:
        patch = mask[y_offs:y_offs + 800, x_offs:x_offs + 1280]
...
def class_centroids_image_from_color(item, tile_size, num_classes, id2trainid):
    ...
    tile_locations = calc_tile_locations(tile_size, image_size) # image_size<tile_size时就没有centroids
    ...
    if id2trainid:
        for k, v in id2trainid.items():
            ...
            if v != 255 and v != -1:
                ...
                c = (mask == nparrayk)[:,:,2]
                mask_copy[a & b & c] = v
    mask = mask_copy # 让trainId不为255和-1的成为mask
    ...
    tile_locations = [tile_locations[0]]
    for x_offs, y_offs in tile_locations: # 计算每一张图片其每个类别的目标的中心坐标(会根据tile_size有细粒度的区分，但是大致能看出来类别数的平衡度)
        patch = mask[y_offs:y_offs + 720, x_offs:x_offs + 1280]
        for class_id in range(num_classes):
            if class_id in patch:
                patch_class = (patch == class_id).astype(int)
                centroid_y, centroid_x = ndimage.measurements.center_of_mass(patch_class)
...
def pooled_class_centroids_all_from_color(items, num_classes, id2trainid, tile_size=1024):
    ...
    class_centroids_item = partial(class_centroids_image_from_color,
                                   ...
                                   tile_size=tile_size) # partial：把一个函数的某些参数给固定住，返回一个新的函数

    centroids = defaultdict(list)
    
    new_centroids = pool.map(class_centroids_item, items) # pool.map()：第一个参数传的是函数，第二个参数传的是数据列表
    pool.close()
    pool.join()
    ...
    return centroids # 相当于存放了每个类别的信息，每个类别的信息包含：包含这个类别的图片们及对应该类别在该图片中的中心坐标们
...
def build_epoch(imgs, centroids, num_classes, class_uniform_pct):
    ...
    return imgs_uniform
# 明白了！class_uniform_pct=0.6表示训练集样本中，40%的样本依然随机选取；60%的样本则为每个class选取num_per_class个包含了该class的样本的集合。可是这就带来了随机性，有些样本可能就选不上，有些样本可能被选很多次（随机选取和class uniform选取两个阶段）。
# 综上，class_uniform_sampling尽量应对了类别不平衡，但是也带来了随机性（class_uniform_pct越高，随机性越高，一个epoch中类别越平衡；class_uniform_pct越低，随机性越低，一个epoch中类别越不平衡），class_uniform_pct过高或过低都有可能影响最终性能。
# 所以，对于那些本来类别就比较平衡的训练集，特别是视频流连续帧采集的数据集，尽量还是不要引入这个class_uniform_sampling。