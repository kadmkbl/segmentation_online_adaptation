...
num_classes = 19
ignore_label = 255
root = cfg.DATASET.C_Driving_DIR
img_postfix = '.jpg'
...
def make_dataset(mode, weather, maxSkip=0, cv_split=0):
    ...
    assert weather in ['cloudy', 'overcast', 'rainy', 'snowy', 'all']
    img_dir_name = 'rgb'
    mask_postfix = '_train_id.png' # 标注的后缀
    if weather == "all":
        for weather in ['cloudy', 'overcast', 'rainy', 'snowy']:
    ...
    # logging.info('C_Driving-{}: {} images'.format(mode, len(items)))
    logging.info('C_Driving-{}-{}: {} images'.format(mode, weather, len(items) + len(aug_items)))
    return items, aug_items


class C_Driving_Uniform(data.Dataset):
    """
    ...
    def __init__(self, mode, weather, maxSkip=0, joint_transform_list=None, sliding_crop=None,
        ...
        json_fn = 'C_Driving_{}_cv{}_tile{}_{}.json'.format(
