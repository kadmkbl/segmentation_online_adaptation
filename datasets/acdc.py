...
def make_cv_splits(img_dir_name, weather):
    ...
    """
    trn_path = os.path.join(root, img_dir_name, 'rgb_anon', weather, 'train')
    val_path = os.path.join(root, img_dir_name, 'rgb_anon', weather, 'val')
...
def make_dataset(mode, weather, maxSkip=0, cv_split=0):
    ...
    assert weather in ['snow', 'rain', 'night', 'fog', 'all']
    img_dir_name = 'rgb_anon_trainvaltest'
    mask_postfix = '_gt_labelIds.png' # 标注的后缀
    if weather == "all":
        for weather in ['snow', 'rain', 'night', 'fog']:
    ...
    logging.info('ACDC-{}-{}: {} images'.format(mode, weather, len(items) + len(aug_items)))
...
class ACDCUniform(data.Dataset):
    ...
    def __init__(self, mode, weather, maxSkip=0, joint_transform_list=None, sliding_crop=None,
        ...
        json_fn = 'ACDC_{}_cv{}_tile{}_{}.json'.format(
        ...
        if self.maxSkip > 0:
            json_fn = 'ACDC_{}_cv{}_tile{}_skip{}.json'.format(
