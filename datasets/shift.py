...
import datasets.shift_labels as shift_labels
import copy
import scipy.misc as m

from config import cfg

trainid_to_name = shift_labels.trainId2name
id_to_trainid = shift_labels.label2trainid
trainid_to_trainid = shift_labels.trainId2trainId
color_to_trainid = shift_labels.color2trainId
num_classes = 14
ignore_label = 255
root = cfg.DATASET.Shift_DIR
img_postfix = '_img_front.jpg'
...
def add_items(items, aug_items, videos, img_path, mask_path, mask_postfix, mode, maxSkip):
    """

    Add More items ot the list from the augmented dataset (Not for ACDC)
    """
...
def make_cv_splits(img_dir_name, weather):
    """
    Create splits of train/val data.
    A split is a lists of videos.
    split0 is aligned with the default ACDC train/val.
    """
    trn_path = os.path.join(root, img_dir_name, weather, 'train')
    val_path = os.path.join(root, img_dir_name, weather, 'val')
...
def make_dataset(mode, weather, maxSkip=0, cv_split=0):
    """
    ...
    assert weather in ['clear_dawn', 'clear_daytime', 'clear_night', 'cloudy_dawn', 'cloudy_daytime', 'cloudy_night',
                       'foggy_dawn', 'foggy_daytime', 'foggy_night', 'overcast_dawn', 'overcast_daytime', 'overcast_night',
                       'rainy_dawn', 'rainy_daytime', 'rainy_night', 'dawn', 'daytime', 'night', 'clear', 'cloudy', 'foggy',
                       'overcast', 'rainy', 'all']
    img_dir_name = 'rgb'
    mask_postfix = '_semseg_front.png' # 标注的后缀
    if weather == "all":
        for weather in ['clear_dawn', 'clear_daytime', 'clear_night', 'cloudy_dawn', 'cloudy_daytime', 'cloudy_night',
                       'foggy_dawn', 'foggy_daytime', 'foggy_night', 'overcast_dawn', 'overcast_daytime', 'overcast_night',
                       'rainy_dawn', 'rainy_daytime', 'rainy_night']:
        # for weather in ['clear_dawn', 'clear_daytime', 'clear_night']:
            img_path = os.path.join(root, img_dir_name, weather) # 图片所处地址（该目录下有train val test；接下来是各个videos；然后才是图片）
            mask_path = os.path.join(root, 'gt', weather) # 标注所处地址（该目录下有train val；接下来是各个videos；然后才是标注）
            cv_splits = make_cv_splits(img_dir_name, weather) # 根据图片所处路径划分train和val的videos
            if mode == 'trainval':
                modes = ['train', 'val']
            else:
                modes = [mode]
            for mode in modes:
                if mode == 'test':
                    ...
                    add_items(items, aug_items, cv_splits[cv_split][mode], img_path, mask_path,
                            mask_postfix, mode, maxSkip) # 根据mode和基于前面划分好的train和val，追踪到叶路径（最终分别存放图片和标注的地方），然后形成“一个图片 + 一个标注”的每个item
    ...
    # logging.info('Shift-{}: {} images'.format(mode, len(items)))
    logging.info('Shift-{}-{}: {} images'.format(mode, weather, len(items) + len(aug_items)))
    return items, aug_items


class SHIFT_Uniform(data.Dataset):
    """
    Please do not use this for AGG
    """

    def __init__(self, mode, weather, maxSkip=0, joint_transform_list=None, sliding_crop=None,
                 ...
                 test=False, coarse_boost_classes=None, image_in=False, extract_feature=False, domain_index=None):
        self.mode = mode
        ...
        self.imgs, self.aug_imgs = make_dataset(mode, self.weather, self.maxSkip, cv_split=self.cv_split)
        assert len(self.imgs), 'Found 0 images, please check the data set'

        # Centroids for fine data
        json_fn = 'Shift_{}_cv{}_tile{}_{}.json'.format(
            self.mode, self.cv_split, self.class_uniform_tile, self.weather)
    ...
    def __getitem__(self, index):
        ...
        #         if mask[i][j] not in class_list:
        #             class_list.append(mask[i][j])
        # print (class_list)
...
class SHIFT(data.Dataset):
    ...
    def __init__(self, mode, weather, maxSkip=0, joint_transform=None, sliding_crop=None,
                 transform=None, target_transform=None, target_aux_transform=None, dump_images=False,
        ...
        self.imgs, _ = make_dataset(mode, weather, self.maxSkip, cv_split=self.cv_split)
        if len(self.imgs) == 0:
