...
augs_root = cfg.DATASET.CITYSCAPES_AUGS_DIR
...
def make_augs_dataset(quality, mode, maxSkip=0, fine_coarse_mult=6, cv_split=0, domain_index=None):
    """
    Assemble list of images + mask files
    ...
    if quality == 'coarse':
        ...
        assert mode in ['train', 'val', 'test', 'trainval']
        img_dir_name = 'leftImg8bit_trainvaltest'
        img_path = os.path.join(augs_root[domain_index], 'leftImg8bit')
        mask_path = os.path.join(root, 'gtFine_trainvaltest', 'gtFine')
        mask_postfix = '_gtFine_labelIds.png'
        cv_splits = make_cv_splits(img_dir_name)
        if mode == 'trainval':
...
class CityScapes(data.Dataset): # 好像作者的报告精度是在val上而不是test上（从所提供的权重文件的名称就可以知道）
...
class CityScapesUniform(data.Dataset):
    ...
    def __init__(self, quality, mode, maxSkip=0, joint_transform_list=None, sliding_crop=None,
                 ...
                 test=False, coarse_boost_classes=None, image_in=False, extract_feature=False, augs=False, domain_index=None):
        self.quality = quality
        self.mode = mode
        self.maxSkip = maxSkip
        ...
        if augs:
            self.imgs, self.aug_imgs = make_augs_dataset(quality, mode, self.maxSkip, cv_split=self.cv_split, domain_index=domain_index)
        else:
