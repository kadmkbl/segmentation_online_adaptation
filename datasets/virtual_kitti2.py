...
import datasets.virtual_kitti2_labels as virtual_kitti2_labels
import copy
import scipy.misc as m

from config import cfg

trainid_to_name = virtual_kitti2_labels.trainId2name
id_to_trainid = virtual_kitti2_labels.label2trainid
color_to_trainid = virtual_kitti2_labels.color2trainId
num_classes = 14
ignore_label = 255
root = cfg.DATASET.Virtual_KITTI2_DIR
img_postfix = '.jpg'
    ...
    for v in videos:
        ...
        for it in v_items:
            item = (os.path.join(img_path, v, it + img_postfix),
                    os.path.join(mask_path, v, it.replace('rgb', 'classgt') + mask_postfix))
            ########################################################
...
def make_dataset(mode, weather, maxSkip=0, cv_split=0):
    ...
    assert weather in ['clone', 'fog', 'morning', 'overcast', 'rain', 'sunset', 'all']
    img_dir_name = 'rgb'
    mask_postfix = '.png' # 标注的后缀
    if weather == "all":
        for weather in ['clone', 'fog', 'morning', 'overcast', 'rain', 'sunset']:
    ...
    # logging.info('Virtual_KITTI2-{}: {} images'.format(mode, len(items)))
    logging.info('Virtual_KITTI2-{}-{}: {} images'.format(mode, weather, len(items) + len(aug_items)))
    return items, aug_items


class VIRTUAL_KITTI2_Uniform(data.Dataset):
    """
    ...
    def __init__(self, mode, weather, maxSkip=0, joint_transform_list=None, sliding_crop=None,
        ...
        json_fn = 'Virtual_KITTI2_{}_cv{}_tile{}_{}.json'.format(
...
class VIRTUAL_KITTI2(data.Dataset):
