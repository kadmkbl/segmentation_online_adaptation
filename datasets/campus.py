...
import datasets.campus_labels as campus_labels
...
def make_cv_splits(img_dir_name, split=None):
    ...
    """
    trn_path = os.path.join(root, split, img_dir_name, 'train')
    val_path = os.path.join(root, split, img_dir_name, 'val')
...
def make_dataset(mode, weather, maxSkip=0, fine_coarse_mult=6, cv_split=0, split=None):
    """
    ...
    img_path = os.path.join(root, split, "images") # 图片所处地址（该目录下有train val test；接下来是各个videos；然后才是图片）
    mask_path = os.path.join(root, split, "labels") # 标注所处地址（该目录下有train val；接下来是各个videos；然后才是标注）
    mask_postfix = '.rgb.png' # 标注的后缀
...
class CampusE1Uniform(data.Dataset):
    ...
    def __init__(self, mode, maxSkip=0, joint_transform_list=None, sliding_crop=None,
                 ...
                 extract_feature=False, split=None):
        self.mode = mode
        ...
        self.imgs, self.aug_imgs = make_dataset(mode, self.maxSkip, cv_split=self.cv_split, split=self.split)
        assert len(self.imgs), 'Found 0 images, please check the data set'

        # Centroids for fine data
        json_fn = 'CampusE1_{}_cv{}_tile{}_{}.json'.format(
            self.mode, self.cv_split, self.class_uniform_tile, self.split)
...
class CampusE1(data.Dataset):
    ...
    def __init__(self, mode, maxSkip=0, joint_transform=None, sliding_crop=None,
                 ...
                 eval_scales=None, eval_flip=False, image_in=False, extract_feature=False, split=None):
        self.mode = mode
        self.maxSkip = maxSkip
        ...
        self.imgs, _ = make_dataset(mode, self.maxSkip, cv_split=self.cv_split, split=self.split)
        if len(self.imgs) == 0:
